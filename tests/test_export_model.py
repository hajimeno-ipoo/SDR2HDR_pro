...
class ExportModelTests(unittest.TestCase):
    ...
    def test_export_v2_torchscript_5ch(self) -> None:
        model = HDRGuidedEnhancementUNet().eval()
        with tempfile.TemporaryDirectory() as temp_dir:
            output_path = Path(temp_dir) / "v2_model.pt"
            export_torchscript(model, output_path)
            loaded = torch.jit.load(str(output_path), map_location="cpu")
            out = loaded(torch.rand(1, 3, 64, 64))
            self.assertEqual(tuple(out.shape), (1, 5, 64, 64))
