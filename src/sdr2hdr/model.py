...
class HDRGuidedEnhancementUNet(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.enc1 = ConvBlock(3, 32, layers=1)
        self.enc2 = ConvBlock(32, 64, layers=2)
        ...
        self.up2 = DecoderBlock(64 + 64, 32)
        self.up1 = DecoderBlock(32 + 32, 32)
        self.map_head = nn.Conv2d(32, 3, kernel_size=1)
        self.luma_head = nn.Conv2d(32, 1, kernel_size=1)
        self.reconstruction_head = nn.Conv2d(32, 1, kernel_size=1)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        maps = torch.tanh(self.map_head(up1))
        target_luma_rel = torch.sigmoid(self.luma_head(up1))
        reconstruction_confidence = torch.sigmoid(self.reconstruction_head(up1))
