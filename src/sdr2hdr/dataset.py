...
class TargetMaps:
    ...
    reconstruction_target: np.ndarray | None = None
    identity_mask: np.ndarray | None = None


def _compute_luma(frame: np.ndarray) -> np.ndarray:
    # prepare_data stores both inputs in linear BT.709 RGB.
...
def derive_target_maps(sdr_linear: np.ndarray, hdr_linear: np.ndarray) -> TargetMaps:
    ...
    # v2 HDR Guidance targets
    target_luma_rel = np.clip(luma_hdr, 0.0, 1.0).astype(np.float32)

    # reconstruction_target: SDR is clipped AND HDR has local structure (gradient)
    sdr_clipped = (luma_sdr > 0.92).astype(np.float32)
    hdr_smooth = cv2.GaussianBlur(hdr_luma_unit, (0, 0), 5.0)
    hdr_gradient = np.abs(hdr_luma_unit - hdr_smooth)
    reconstruction_target = np.clip(
        sdr_clipped * np.clip(hdr_gradient / 0.05, 0.0, 1.0),
        0.0,
        1.0,
    ).astype(np.float32)

    # identity_mask: normal non-clipped regions have high identity preservation weight
    identity_mask = np.clip(1.0 - clip_mask, 0.0, 1.0).astype(np.float32)
    ...
    return TargetMaps(
        ...
        clip_mask=clip_mask,
        target_luma_rel=target_luma_rel,
...
def _sample_content_name(path: Path) -> str:
    """Read the manifest group saved in an NPZ, with a filename fallback."""
    with np.load(path, allow_pickle=False) as sample:
        for key in ("content_name", "group_name"):
...
class HDRSDRPairDataset(Dataset):
    def __init__(
        ...
        self.content_names = [_sample_content_name(path) for path in self.paths]
    ...
    def __getitem__(self, index: int) -> dict[str, torch.Tensor]:
        with np.load(self.paths[index], allow_pickle=False) as sample:
            ...
            peak_nits = float(sample["peak_nits"]) if "peak_nits" in sample else None
        if self.training:
        ...
        target_luma_rel = (
            targets.target_luma_rel[None]
            if targets.target_luma_rel is not None
            else np.zeros((1, sdr_linear.shape[0], sdr_linear.shape[1]), dtype=np.float32)
        )
        reconstruction_target = (
            targets.reconstruction_target[None]
            if targets.reconstruction_target is not None
            else np.zeros((1, sdr_linear.shape[0], sdr_linear.shape[1]), dtype=np.float32)
        )
        identity_mask = (
            targets.identity_mask[None]
            if targets.identity_mask is not None
            else np.ones((1, sdr_linear.shape[0], sdr_linear.shape[1]), dtype=np.float32)
        )
        ...
        item: dict[str, torch.Tensor] = {
            ...
            "target_luma_rel": torch.from_numpy(target_luma_rel).to(torch.float32),
            "reconstruction_target": torch.from_numpy(reconstruction_target).to(torch.float32),
            "identity_mask": torch.from_numpy(identity_mask).to(torch.float32),
            "clip_mask": torch.from_numpy(targets.clip_mask[None]).to(torch.float32),
        }
        if peak_nits is not None:
            item["peak_nits"] = torch.tensor(peak_nits, dtype=torch.float32)
        return item
