...
# BT.2020/D65 to AP1/ACES white, with Bradford chromatic adaptation.
# This changes RGB coordinates, not display-referred values into scene exposure.
REC2020_TO_ACESCG = np.array(
    [
        [0.974894977924, 0.019599108637, 0.005505913439],
        [0.002179562798, 0.995535468893, 0.002284968309],
        [0.004797239684, 0.024532016635, 0.970670743682],
    ],
    dtype=np.float32,
)
...
def apply_cinema_tone(frame_linear: np.ndarray) -> np.ndarray:
    """Apply a restrained toe and shoulder while preserving mid-gray."""
    luma = np.maximum(compute_luma_bt709(frame_linear), 0.0)
    toe = (luma + 0.08) / (luma + 0.12)
    toe_reference = (1.0 + 0.08) / (1.0 + 0.12)
    mapped_luma = luma * toe / toe_reference
    mapped_luma = mapped_luma / (1.0 + 0.18 * np.maximum(luma - 1.0, 0.0))
    gain = mapped_luma / np.maximum(luma, 1e-4)
    return np.clip(frame_linear * gain[..., None], 0.0, 4.0)
...
class ProcessorConfig:
    ...
    tone: str = "reference"
    diffuse_white_nits: float = 203.0
    ...
    hdr_guidance: str = "auto"  # "auto", "on", "off"
    luminance_guidance_strength: float = 0.70
    reconstruction_strength: float = 0.60
    # Mixing unaligned previous pixels leaves brightness trails on moving objects.
    # This setting is used only when v2 HDR guidance is active.
    target_luma_temporal_strength: float = 0.0
    gamut_expansion: float = 0.0
    output_color_space: str = "bt2020_pq"
...
class TemporalState:
    def __init__(self) -> None:
        ...
        self.prev_target_base_small: np.ndarray | None = None
        self.prev_target_base_small_t: torch.Tensor | None = None
    ...
    def reset(self, luma_mean: float, luma_small: np.ndarray, chroma_small: np.ndarray) -> float:
        ...
        self.prev_target_base_small = None
        self.prev_target_base_small_t = None
    ...
    def update_target_base_numpy(self, target_base: np.ndarray, alpha: float, scene_cut: bool) -> np.ndarray:
        if self.prev_target_base_small is None or scene_cut:
            self.prev_target_base_small = target_base.copy()
            return target_base
        a = np.clip(alpha, 0.0, 0.99)
        self.prev_target_base_small = self.prev_target_base_small * a + target_base * (1.0 - a)
        return self.prev_target_base_small

    def update_target_base_torch(self, target_base_t: torch.Tensor, alpha: float, scene_cut: bool) -> torch.Tensor:
        if self.prev_target_base_small_t is None or scene_cut:
            self.prev_target_base_small_t = target_base_t.clone()
            return target_base_t
        a = float(np.clip(alpha, 0.0, 0.99))
        self.prev_target_base_small_t = self.prev_target_base_small_t * a + target_base_t * (1.0 - a)
        return self.prev_target_base_small_t


def compute_luma(frame_linear: np.ndarray) -> np.ndarray:
    """Luminance of linear BT.2020 RGB (the output working space)."""
    return 0.2627 * frame_linear[..., 0] + 0.6780 * frame_linear[..., 1] + 0.0593 * frame_linear[..., 2]


def compute_luma_bt709(frame_linear: np.ndarray) -> np.ndarray:
    """Luminance of linear BT.709 RGB (SDR input and model targets)."""
    return 0.2126 * frame_linear[..., 0] + 0.7152 * frame_linear[..., 1] + 0.0722 * frame_linear[..., 2]
...
def estimate_noise_mask(
    ...
    if threshold is None:
        ...
        if blur is None:
            blur = cv2.GaussianBlur(np.clip(compute_luma_bt709(frame_or_luma), 0.0, 1.0).astype(np.float32), (0, 0), 1.2)
...
class SDRToHDRProcessor:
    def __init__(self, config: ProcessorConfig, enhancer: BaseEnhancer | None = None) -> None:
        ...
        self.max_cll_nits: float = 0.0
        self.max_frame_average_nits: float = 0.0
        self.sum_frame_average_nits: float = 0.0
        self.measured_frame_count: int = 0
    ...
    def _torch_compute_luma_bt709(self, frame_linear: torch.Tensor) -> torch.Tensor:
        return frame_linear[..., 0] * 0.2126 + frame_linear[..., 1] * 0.7152 + frame_linear[..., 2] * 0.0722
    ...
    def _torch_apply_cinema_tone(self, frame_linear: torch.Tensor) -> torch.Tensor:
        luma = torch.clamp(self._torch_compute_luma_bt709(frame_linear), min=0.0)
        toe = (luma + 0.08) / (luma + 0.12)
        toe_reference = (1.0 + 0.08) / (1.0 + 0.12)
        mapped_luma = luma * toe / toe_reference
        mapped_luma = mapped_luma / (1.0 + 0.18 * torch.clamp(luma - 1.0, min=0.0))
        gain = mapped_luma / torch.clamp(luma, min=1e-4)
        return torch.clamp(frame_linear * gain[..., None], 0.0, 4.0)
    ...
    def _torch_blur(self, image: torch.Tensor, kernel: int = 5) -> torch.Tensor:
        ...
        padding = kernel // 2
        return F.avg_pool2d(
            image[None, None], kernel, stride=1, padding=padding, count_include_pad=False,
        ).squeeze(0).squeeze(0)
    ...
    def _torch_laplacian(self, image: torch.Tensor) -> torch.Tensor:
        ...
        padded = F.pad(image[None, None], (1, 1, 1, 1), mode="replicate")
        return F.conv2d(padded, self._laplacian_kernel_t[None, None]).squeeze(0).squeeze(0)
    ...
    def _torch_heuristic_maps(self, frame_linear_t: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        luminance = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), 0.0, 1.0)
    ...
    def render_frame_nits_torch(self, frame_bgr8: np.ndarray) -> tuple[torch.Tensor, torch.Tensor]:
        """Shared production/training render before PQ quantization.

        Returns peak-clamped Rec.2020 nits and relative Rec.2020 RGB. Input-only
        scene statistics update the usual state; model-dependent pixels retain
        gradients when the enhancer was constructed with for_training().
        """
        assert torch is not None
        ...
        luma_t = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), 0.0, 1.0)
        ...
        if self.config.disable_auto_exposure:
            exposure = 1.0
        frame_linear_t = torch.clamp(frame_linear_t * exposure, 0.0, 2.0)

        # Get guidance/maps from enhancer
        target_luma_rel_t: torch.Tensor | None = None
        recon_conf_t: torch.Tensor | None = None
        reference_nits = self.config.peak_nits
        if isinstance(self.enhancer, TorchMapEnhancer):
            guidance_t = self.enhancer.estimate_guidance_torch(frame_linear_t)
            maps_expansion = guidance_t.expansion
            maps_contrast = guidance_t.contrast
            maps_protection = guidance_t.protection
            ...
            if guidance_t.has_hdr_guidance:
                if guidance_t.reference_nits is not None:
                    reference_nits = guidance_t.reference_nits
                target_luma_rel_t = guidance_t.target_luma_rel.to(frame_linear_t.device)  # type: ignore[union-attr]
                recon_conf_t = guidance_t.reconstruction_confidence.to(frame_linear_t.device)  # type: ignore[union-attr]
            skin_mask = self._torch_skin_mask(frame_linear_t)
        elif isinstance(self.enhancer, HeuristicEnhancer):
            ...
            guidance = self.enhancer.estimate_guidance(frame_linear_np)
            maps_expansion = torch.from_numpy(guidance.expansion).to(self.torch_device, dtype=torch.float32)
            maps_contrast = torch.from_numpy(guidance.contrast).to(self.torch_device, dtype=torch.float32)
            maps_protection = torch.from_numpy(guidance.protection).to(self.torch_device, dtype=torch.float32)
            if guidance.has_hdr_guidance:
                if guidance.reference_nits is not None:
                    reference_nits = guidance.reference_nits
                target_luma_rel_t = torch.from_numpy(guidance.target_luma_rel).to(self.torch_device, dtype=torch.float32)  # type: ignore[arg-type]
                recon_conf_t = torch.from_numpy(guidance.reconstruction_confidence).to(self.torch_device, dtype=torch.float32)  # type: ignore[arg-type]
            skin_mask = torch.from_numpy(estimate_skin_mask(frame_linear_np)).to(self.torch_device, dtype=torch.float32)

        use_hdr_guidance = (
            self.config.hdr_guidance != "off"
            and target_luma_rel_t is not None
            and recon_conf_t is not None
        )
        ...
        luma_t = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), 0.0, 2.0)
        ...
        legacy_protected = torch.clamp(protected + clipped_white_mask * self.config.clipped_white_protection, 0.0, 1.0)

        skin_ratio = skin_mask.mean().item()
        specular_ratio = specular_ratio = specular_mask.mean().item()
        ...
        legacy_protected = legacy_protected.to(torch.float32)
        guidance_protected = protected.to(torch.float32)
        ...
        if use_hdr_guidance and target_luma_rel_t is not None and recon_conf_t is not None:
            # v2 HDR Guidance Pipeline
            anchor_nits = resolve_anchor_nits(self.config.tone, self.config.peak_nits, self.config.diffuse_white_nits)
            model_target_linear = target_rel_to_linear_torch(target_luma_rel_t, reference_nits, anchor_nits)
            
            # Reconstruction gate for clipped highlights
            recon_gate = build_reconstruction_gate_torch(
                clipped_white_mask.to(torch.float32),
                recon_conf_t,
                self.config.reconstruction_strength,
                guidance_protected,
            )
            
            # Guidance weight combines base strength with reconstruction bonus
            guidance_weight = torch.clamp(
                self.config.luminance_guidance_strength * ai_gate + recon_gate,
                0.0,
                1.0,
            ) * (1.0 - guidance_protected)
            
            legacy_luma = torch.clamp(self._torch_compute_luma_bt709(noise_limited), 0.0, 4.0)
            target_luma = blend_target_luma_torch(legacy_luma, model_target_linear, guidance_weight)
            
            # Temporal stabilization on low-frequency base target
            target_base = self._torch_blur(target_luma, 11)
            target_detail = target_luma - target_base
            stable_base = self.state.update_target_base_torch(
                target_base,
                self.config.target_luma_temporal_strength,
                scene_cut,
            )
            stable_target_luma = torch.clamp(stable_base + target_detail, 0.0, 10.0)
            
            # Apply luminance gain while strictly preserving original RGB chromaticity
            luma_safe = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), min=1e-4)
            gain = torch.clamp(stable_target_luma / luma_safe, 0.05, 8.0)
            guided_linear = torch.clamp(frame_linear_t * gain[..., None], 0.0, 4.0)
            
            frame_linear_t = (
                guided_linear * (1.0 - guidance_protected[..., None] * self.config.shadow_rolloff)
                + frame_linear_t * guidance_protected[..., None] * self.config.shadow_rolloff
            )
        else:
            # Legacy pipeline
            frame_linear_t = (
                noise_limited * (1.0 - legacy_protected[..., None] * self.config.shadow_rolloff)
                + frame_linear_t * legacy_protected[..., None] * self.config.shadow_rolloff
            )

        relit_luma_t = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), 0.0, 2.0)
        ...
        if abs(self.config.saturation - 1.0) > 1e-4:
            luma_32 = torch.clamp(self._torch_compute_luma_bt709(frame_linear_t), 0.0, 4.0)
            frame_linear_t = torch.clamp(luma_32[..., None] + (frame_linear_t - luma_32[..., None]) * self.config.saturation, 0.0, 4.0)
        if self.config.tone == "cinema":
            frame_linear_t = self._torch_apply_cinema_tone(frame_linear_t)

        # Convert to Rec.2020 and PQ with strict peak nit clamping
        frame_2020_t = torch.clamp(torch.matmul(frame_linear_t, self._rec2020_t.T), min=0.0)
        if self.config.gamut_expansion > 0.0:
            expansion = torch.clamp(maps_expansion, 0.0, 1.0)
            luma_2020 = self._torch_compute_luma(frame_2020_t)
            chroma_gain = 1.0 + float(self.config.gamut_expansion) * expansion
            frame_2020_t = torch.clamp(
                luma_2020[..., None] + (frame_2020_t - luma_2020[..., None]) * chroma_gain[..., None],
                min=0.0,
            )
        anchor_nits = resolve_anchor_nits(self.config.tone, self.config.peak_nits, self.config.diffuse_white_nits)
        frame_nits_t = clamp_to_peak_torch(frame_2020_t * anchor_nits, self.config.peak_nits)
        return frame_nits_t, frame_2020_t

    def _process_frame_torch(self, frame_bgr8: np.ndarray) -> np.ndarray:
        frame_nits_t, frame_2020_t = self.render_frame_nits_torch(frame_bgr8)
        
        # CTA-861.3 content light levels use max(R, G, B), not CIE Y.
        max_rgb_nits_t = torch.amax(frame_nits_t, dim=-1)
        frame_max = float(torch.amax(max_rgb_nits_t).item())
        frame_mean = float(torch.mean(max_rgb_nits_t).item())
        self.max_cll_nits = max(self.max_cll_nits, frame_max)
        self.max_frame_average_nits = max(self.max_frame_average_nits, frame_mean)
        self.sum_frame_average_nits += frame_mean
        self.measured_frame_count += 1

        if self.config.output_color_space == "acescg_linear":
            acescg_t = torch.matmul(frame_2020_t, torch.from_numpy(REC2020_TO_ACESCG).to(self.torch_device).T)
    ...
    def process_frame(self, frame_bgr8: np.ndarray) -> np.ndarray:
        if self.torch_device is not None:
            with torch.inference_mode():
                result = self._process_frame_torch(frame_bgr8)
            result_shape = result.shape[:2] if result.dtype == np.uint16 else result.shape[1:]
            if result_shape != frame_bgr8.shape[:2]:
                height, width = frame_bgr8.shape[:2]
                if result.dtype == np.uint16:
                    result = cv2.resize(result, (width, height), interpolation=cv2.INTER_LINEAR)
                else:
                    result = np.stack(
                        [cv2.resize(channel, (width, height), interpolation=cv2.INTER_LINEAR) for channel in result],
                        axis=0,
                    )
            return result
        original_height, original_width = frame_bgr8.shape[:2]
        ...
        luma = np.clip(compute_luma_bt709(frame_linear), 0.0, 1.0)
        chroma = compute_chroma(frame_linear)
        ...
        guidance = self.enhancer.estimate_guidance(frame_linear)
        use_hdr_guidance = (self.config.hdr_guidance != "off") and guidance.has_hdr_guidance

        skin_mask = estimate_skin_mask(frame_linear)
        luma = np.clip(compute_luma_bt709(frame_linear), 0.0, 1.5)
        ...
        highlight_mask = np.clip(specular_mask * 0.75 + sky_mask * 0.35 + guidance.expansion * 0.25, 0.0, 1.0)
        highlight_mask = highlight_mask * (1.0 - clipped_white_mask * self.config.clipped_white_protection)
        ...
        protected = np.clip(
            self.config.skin_protection * skin_mask
            + 0.25 * guidance.protection
            + self.config.subtitle_protection * subtitle_mask
            + 0.35 * noise_mask,
            0.0,
            1.0,
        )
        legacy_protected = np.clip(protected + clipped_white_mask * self.config.clipped_white_protection, 0.0, 1.0)
        ...
        if use_hdr_guidance and guidance.target_luma_rel is not None and guidance.reconstruction_confidence is not None:
            # v2 HDR Guidance Pipeline
            anchor_nits = resolve_anchor_nits(self.config.tone, self.config.peak_nits, self.config.diffuse_white_nits)
            reference_nits = guidance.reference_nits if guidance.reference_nits is not None else self.config.peak_nits
            model_target_linear = target_rel_to_linear(guidance.target_luma_rel, reference_nits, anchor_nits)
            
            recon_gate = build_reconstruction_gate(
                clipped_white_mask,
                guidance.reconstruction_confidence,
                self.config.reconstruction_strength,
                protected,
            )
            ...
            legacy_luma = np.clip(compute_luma_bt709(noise_limited), 0.0, 4.0)
            target_luma = blend_target_luma(legacy_luma, model_target_linear, guidance_weight)
            
            # Temporal stabilization on low-frequency base target
            target_base = cv2.GaussianBlur(target_luma.astype(np.float32), (0, 0), 5.0)
            target_detail = target_luma - target_base
            stable_base = self.state.update_target_base_numpy(
                target_base,
                self.config.target_luma_temporal_strength,
                scene_cut,
            )
            stable_target_luma = np.clip(stable_base + target_detail, 0.0, 10.0)
            
            # Apply luminance gain
            luma_safe = np.maximum(compute_luma_bt709(frame_linear), 1e-4)
            gain = np.clip(stable_target_luma / luma_safe, 0.05, 8.0)
            guided_linear = np.clip(frame_linear * gain[..., None], 0.0, 4.0)
            
            frame_linear = (
                guided_linear * (1.0 - protected[..., None] * self.config.shadow_rolloff)
            ...
            frame_linear = (
                noise_limited * (1.0 - legacy_protected[..., None] * self.config.shadow_rolloff)
                + frame_linear * legacy_protected[..., None] * self.config.shadow_rolloff
            )

        relit_luma = np.clip(compute_luma_bt709(frame_linear), 0.0, 2.0)
        detail_fn = fast_detail_boost if self.config.fast_mode else bilateral_detail_boost
        boosted_luma = detail_fn(
            relit_luma,
            (self.config.detail_boost * (0.35 + 0.65 * guidance.contrast * ai_gate))
        ...
        if abs(self.config.saturation - 1.0) > 1e-4:
            luma_lin = np.clip(compute_luma_bt709(frame_linear), 0.0, 4.0)
            frame_linear = np.clip(luma_lin[..., None] + (frame_linear - luma_lin[..., None]) * self.config.saturation, 0.0, 4.0)
        if self.config.tone == "cinema":
            frame_linear = apply_cinema_tone(frame_linear)

        frame_2020 = apply_matrix(frame_linear, REC709_TO_REC2020)
        if self.config.gamut_expansion > 0.0:
            expansion = np.clip(guidance.expansion, 0.0, 1.0)
            luma_2020 = compute_luma(frame_2020)
            chroma_gain = 1.0 + self.config.gamut_expansion * expansion
            frame_2020 = np.clip(
                luma_2020[..., None] + (frame_2020 - luma_2020[..., None]) * chroma_gain[..., None],
                0.0,
                None,
            )
        anchor_nits = resolve_anchor_nits(self.config.tone, self.config.peak_nits, self.config.diffuse_white_nits)
        frame_nits = clamp_to_peak(frame_2020 * anchor_nits, self.config.peak_nits)

        # CTA-861.3 content light levels use max(R, G, B), not CIE Y.
        max_rgb_nits = np.max(frame_nits, axis=-1)
        frame_max = float(np.max(max_rgb_nits))
        frame_mean = float(np.mean(max_rgb_nits))
        self.max_cll_nits = max(self.max_cll_nits, frame_max)
        self.max_frame_average_nits = max(self.max_frame_average_nits, frame_mean)
        self.sum_frame_average_nits += frame_mean
        self.measured_frame_count += 1
        ...
        if self.config.output_color_space == "acescg_linear":
            frame_acescg = apply_matrix(frame_2020, REC2020_TO_ACESCG)
            if frame_acescg.shape[:2] != frame_bgr8.shape[:2]:
                frame_acescg = cv2.resize(
                    frame_acescg, (original_width, original_height), interpolation=cv2.INTER_LINEAR,
                )
        ...
        if frame_16.shape[:2] != frame_bgr8.shape[:2]:
            frame_16 = cv2.resize(frame_16, (frame_bgr8.shape[1], frame_bgr8.shape[0]), interpolation=cv2.INTER_LINEAR)
        return frame_16

    def get_measured_hdr_metadata(self) -> tuple[float, float]:
        """Return measured (max_cll, max_fall) in nits."""
        max_cll = float(self.max_cll_nits)
        max_fall = float(self.max_frame_average_nits)
