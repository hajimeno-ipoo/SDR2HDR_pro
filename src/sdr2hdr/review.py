...
def compute_log_luma_mae(
    pred_linear: np.ndarray,
    target_linear: np.ndarray,
    eps: float = 1e-4,
) -> float:
    """Compute log-luminance Mean Absolute Error between predicted and target HDR linear frames."""
    luma_pred = 0.2627 * pred_linear[..., 0] + 0.6780 * pred_linear[..., 1] + 0.0593 * pred_linear[..., 2]
    luma_tgt = 0.2627 * target_linear[..., 0] + 0.6780 * target_linear[..., 1] + 0.0593 * target_linear[..., 2]
    log_pred = np.log(np.maximum(luma_pred, 0.0) + eps)
    log_tgt = np.log(np.maximum(luma_tgt, 0.0) + eps)
    return float(np.mean(np.abs(log_pred - log_tgt)))


def compute_clipped_region_luma_mae(
    pred_linear: np.ndarray,
    target_linear: np.ndarray,
    sdr_linear: np.ndarray,
    clip_threshold: float = 0.9,
    eps: float = 1e-4,
) -> float:
    """Compute log-luma MAE exclusively in SDR clipped highlights."""
    luma_sdr = 0.2627 * sdr_linear[..., 0] + 0.6780 * sdr_linear[..., 1] + 0.0593 * sdr_linear[..., 2]
    mask = luma_sdr >= clip_threshold
    if not np.any(mask):
        return 0.0
    luma_pred = 0.2627 * pred_linear[..., 0] + 0.6780 * pred_linear[..., 1] + 0.0593 * pred_linear[..., 2]
    luma_tgt = 0.2627 * target_linear[..., 0] + 0.6780 * target_linear[..., 1] + 0.0593 * target_linear[..., 2]
    log_pred = np.log(np.maximum(luma_pred[mask], 0.0) + eps)
    log_tgt = np.log(np.maximum(luma_tgt[mask], 0.0) + eps)
    return float(np.mean(np.abs(log_pred - log_tgt)))


def compute_chroma_preservation_error(
    pred_linear: np.ndarray,
    original_linear: np.ndarray,
    normal_mask: np.ndarray | None = None,
) -> float:
    """Compute chromaticity ratio preservation error in non-clipped regions."""
    orig_sum = np.maximum(np.sum(original_linear, axis=-1, keepdims=True), 1e-4)
    pred_sum = np.maximum(np.sum(pred_linear, axis=-1, keepdims=True), 1e-4)
    orig_chroma = original_linear / orig_sum
    pred_chroma = pred_linear / pred_sum
    diff = np.abs(orig_chroma - pred_chroma)
    if normal_mask is not None:
        if not np.any(normal_mask):
            return 0.0
        return float(np.mean(diff[normal_mask]))
    return float(np.mean(diff))


def compute_temporal_luma_delta(frames_linear: list[np.ndarray]) -> float:
    """Compute mean inter-frame luminance difference to assess temporal flicker."""
    if len(frames_linear) < 2:
        return 0.0
    deltas: list[float] = []
    prev_luma = 0.2627 * frames_linear[0][..., 0] + 0.6780 * frames_linear[0][..., 1] + 0.0593 * frames_linear[0][..., 2]
    for frame in frames_linear[1:]:
        curr_luma = 0.2627 * frame[..., 0] + 0.6780 * frame[..., 1] + 0.0593 * frame[..., 2]
        deltas.append(float(np.mean(np.abs(curr_luma - prev_luma))))
        prev_luma = curr_luma
